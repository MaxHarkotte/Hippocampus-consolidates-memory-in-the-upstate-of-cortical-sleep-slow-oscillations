...
for iRec =  1 :size(selection,1)
    ...
    if rec_info.StimProtocol == "No_stim"
        no_stim_snippet = rec_filtered.trial{1,1}(online_SO.sample(no_stim_SO_idx)-5*fs:online_SO.sample(no_stim_SO_idx +1)+5*fs);

        SO_times  = online_SO(find(strcmp(online_SO.type, "StimON")),:);
        inBetween = SO_times.sample(SO_times.sample >= online_SO.sample(no_stim_SO_idx)-5*fs & SO_times.sample<= online_SO.sample(no_stim_SO_idx +1)+5*fs);
        no_stim_SOs = inBetween - (online_SO.sample(no_stim_SO_idx) - 5*fs);

    elseif rec_info.StimProtocol == "SO_up_in_phase"
        in_phase_snippet = rec_filtered.trial{1,1}(online_SO.sample(in_phase_SO_idx)-5*fs:online_SO.sample(in_phase_SO_idx +1)+5*fs);

        SO_times  = online_SO(find(strcmp(online_SO.type, "StimON")),:);
        inBetween = SO_times.sample(SO_times.sample >= online_SO.sample(in_phase_SO_idx)-5*fs & SO_times.sample<= online_SO.sample(in_phase_SO_idx +1)+5*fs);
        in_phase_SOs = inBetween - (online_SO.sample(in_phase_SO_idx) - 5*fs);
    ...
    elseif rec_info.StimProtocol == "SO_delayed"
        ...
        delayed_stims_dur = delayed_stims_off - delayed_stims; 
...
fig1 = figure('Color','w','Position',[200 200 1000 500]);
...
hold on
for i = 1:length(delayed_stims)
    rectangle('Position', [delayed_stims(i) 400 delayed_stims_dur(i) 30], 'FaceColor', 'r', 'EdgeColor', 'none');
end
for i = 1:length(delayed_SOs)
    xline(delayed_SOs(i), '--', 'LineWidth', 1);
end
