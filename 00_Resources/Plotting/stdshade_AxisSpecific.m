...
function [lineOut, fillOut] = stdshade_AxisSpecific(amatrix,alpha,acolor,F,smth,ax1)
...
if exist('ax1','var'); if isempty(ax1); ax1=gca; end
else ax1=gca; %no smoothing by default
end
...
hold on;
if ischar(acolor)
    lineOut = plot(ax1,F,amean,acolor,'linewidth',1.5); %% change color or linewidth to adjust mean line
else
    lineOut = plot(ax1,F,amean,'k','linewidth',1.5); %% change color or linewidth to adjust mean line
    lineOut.Color = acolor;
end
if check
    hold off;
end
